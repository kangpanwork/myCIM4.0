...
public class LotForceProcessMoveExecutor implements PostProcessExecutor {
    ...
    @Autowired
    public LotForceProcessMoveExecutor(ILotMethod lotMethod, ILotProcessOperationService lotProcessOperationService) {
    ...
    public PostProcessTask.Result doExecute(PostProcessTask.Param param) {
        ...
        if (isMoveOutReq) {
            ...
            if (CimStringUtils.equals(lotHoldState, BizConstant.CIMFW_LOT_HOLDSTATE_ONHOLD)) {
                ...
                List<Infos.LotHoldListAttributes> lotHoldListAttributes = lotMethod.lotFillInTxTRQ005DR(objCommon, lotID);
                if (CimArrayUtils.getSize(lotHoldListAttributes) == 1) {
                    String reasonCodeID = BizConstant.SP_REASON_FORCECOMPHOLD;
                    if (log.isDebugEnabled()) {
                        log.debug("Since there is RNHR Hold before ForceMoveOut, there is no need to skip process move");
                    }
                    boolean foundForceComp = Optional.ofNullable(ObjectIdentifier.
                            fetchValue(lotHoldListAttributes.get(0).getReasonCodeID()))
                            .map(code -> code.equals(reasonCodeID)).orElse(false);
                    if (!foundForceComp) {
                        return PostProcessTask.success();
                    }
                }
