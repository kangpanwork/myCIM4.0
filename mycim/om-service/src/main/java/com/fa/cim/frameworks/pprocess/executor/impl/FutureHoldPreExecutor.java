...
/**
 * executor for future hold in pre phase
 *
 * @author Yuri
 */
@Slf4j
@PostProcessTaskHandler(isNextOperationRequired = true)
public class FutureHoldPreExecutor implements PostProcessExecutor {

    private final ILotMethod lotMethod;
    private final IProcessControlService processControlService;

    @Autowired
    public FutureHoldPreExecutor(ILotMethod lotMethod, IProcessControlService processControlService) {
    ...
    public PostProcessTask.Result doExecute(PostProcessTask.Param param) {
        ...
        if (CimStringUtils.equals(lotState, CIMStateConst.CIM_LOT_STATE_ACTIVE)) {
            ...
            //-----------------------------------------------------------------
            processControlService.sxFutureHoldPreByPostProcessReq(objCommon, lotID, "");
