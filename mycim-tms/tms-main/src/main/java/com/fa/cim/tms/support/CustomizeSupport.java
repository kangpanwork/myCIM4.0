...
import com.fa.cim.tms.entity.BaseEntity;
...
public class CustomizeSupport {
    ...
    public <E extends BaseEntity> long count(E entity) {
        Assert.notNull(entity, "The query main example cannot be null");
        Class<? extends BaseEntity> mainType = entity.getClass();

        Table mainTable = mainType.getAnnotation(Table.class);
        Assert.notNull(mainTable, "No @Table declared on main entity");

        String mainTableName = mainTable.name();

        StringBuilder sql = new StringBuilder(String.format("SELECT COUNT(M.ID) FROM %s M, WHERE 1 = 1 ", mainTableName));

        return this.count(sql.toString(), CimReflectionUtils.allFields(mainType).stream()
                .map(field -> processValue("M", entity, sql, field))
                .filter(Objects::nonNull).toArray());
    }

    private <M extends BaseEntity> Object processValue(String prefix, M example, StringBuilder sqlBuilder, Field field) {
        ReflectionUtils.makeAccessible(field);
        Object value = ReflectionUtils.getField(field, example);
        if (null != value) {
            sqlBuilder.append(" AND ").append(prefix).append(".");
            Column column = field.getAnnotation(Column.class);
            if (null == column) {
