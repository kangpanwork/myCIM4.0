...
@RestController
@RequestMapping("/mcs")
public class DateAndTimeReqController {

    @Autowired
    private IToTmsRemoteManager toTmsRemoteManager;

    @RequestMapping(value = "/data_and_time/req", method = RequestMethod.POST)
    public Response dateAndTimeReq(@RequestBody Params.DateAndTimeReqRestParmas dateAndTimeReqRestParmas) {
        final String transactionID = TransactionIDEnum.TM11.getValue();
        ThreadContextHolder.setTransactionId(transactionID);
        log.info("mq MCS ->TMS callTmsDateAndTimeReq request {}", JSON.toJSONString(dateAndTimeReqRestParmas));
        Response response = null;
        try {
            response = toTmsRemoteManager.callTmsDateAndTimeReq(dateAndTimeReqRestParmas);
        } catch(CimIntegrationException e) {
            Validations.check(true,new Code((int) e.getCode(),e.getMessage()));
        }
        log.info("mq MCS ->TMS callTmsDateAndTimeReq response {}" + JSON.toJSONString(response));
        return response;
    }
}
