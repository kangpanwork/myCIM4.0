...
public class DurableOperationStartHistoryService {
     ...
     * @date 2019/7/1 10:38
     */
    public Infos.DurableOperationStartEventRecord getEventData(String id) {
        String sql="Select * from OMEVDURMVI where id=?";
        List<Map> sqlDatas = baseCore.queryAllForMap(sql, id);
        Infos.DurableOperationStartEventRecord theEventData=new Infos.DurableOperationStartEventRecord();
    ...
    public List<Infos.UserDataSet> allUserDataSets(String refKey) {
        String sql="SELECT * FROM OMEVDURMVI_CDA WHERE REFKEY=?";
