...
public class DurableControlJobStatusChangeHistoryService {
     ...
     * @date 2019/7/1 10:38
     */
    public Infos.DurableControlJobStatusChangeEventRecord getEventData(String id) {
        String sql="Select * from OMEVDCJSC where id=?";
        List<Map> sqlDatas = baseCore.queryAllForMap(sql, id);
        Infos.DurableControlJobStatusChangeEventRecord theEventData=new Infos.DurableControlJobStatusChangeEventRecord();
        Infos.EventData eventCommon = new Infos.EventData();
        theEventData.setEventCommon(eventCommon);
        List<Infos.DurableControlJobStatusChangeDurableEventData> durables=new ArrayList<>();
        theEventData.setDurables(durables);
        for (Map<String,Object> sqlData:sqlDatas) {
            theEventData.setDurableCtrlJobID(convert(sqlData.get("DCJ_ID")));
            theEventData.setDurableCtrlJobState(convert(sqlData.get("DCJ_STATE")));
            theEventData.setEqpID(convert(sqlData.get("EQP_ID")));
            theEventData.setEqpDescription(convert(sqlData.get("EQP_DESC")));

            sql="SELECT * FROM OMEVDCJSC_DRBLS WHERE REFKEY=?";
            List<Map> sqlDurables=baseCore.queryAllForMap(sql,id);

            for (Map sqlDurable:sqlDurables) {
                Infos.DurableControlJobStatusChangeDurableEventData durable=new Infos.DurableControlJobStatusChangeDurableEventData();
                durables.add(durable);

                durable.setDurableID(convert(sqlDurable.get("DRBL_ID")));
                durable.setDurableCategory(convert(sqlDurable.get("DRBL_CATEGORY")));
                durable.setProdSpecID(convert(sqlDurable.get("PROD_ID")));
                durable.setMainPDID(convert(sqlDurable.get("PROCESS_ID")));
                durable.setOpeNo(convert(sqlDurable.get("OPE_NO")));
                durable.setPdID(convert(sqlDurable.get("STEP_ID")));
                durable.setOpePassCount(convertL(sqlDurable.get("OPE_PASS_COUNT")));
                durable.setPdName(convert(sqlDurable.get("STEP_NAME")));
            }
    ...
    public List<Infos.UserDataSet> allUserDataSets(String refKey) {
        String sql="SELECT * FROM OMEVDCJSC_CDA WHERE REFKEY=?";
