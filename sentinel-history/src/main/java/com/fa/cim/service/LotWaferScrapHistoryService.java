...
public class LotWaferScrapHistoryService {
     ...
     */
    public Infos.LotWaferScrapEventRecord getEventData(String id) {
        String sql="Select * from OMEVWFSCR where id=?";
        List<Map> sqlDatas = baseCore.queryAllForMap(sql, id);
        Infos.LotWaferScrapEventRecord theEventData=new Infos.LotWaferScrapEventRecord();
        ...
        List<Infos.WaferScrapEventData> scrapWafers=new ArrayList<>();
        theEventData.setScrapWafers(scrapWafers);
        ...
        for (Map<String,Object> sqlData:sqlDatas) {
            ...
            sql="SELECT * FROM OMEVWFSCR_CURWF WHERE REFKEY=?";
            ...
            sql="SELECT * FROM OMEVLWFMV_SCRWFR WHERE REFKEY=?";
            List<Map> sqlScrapWafers = baseCore.queryAllForMap(sql, id);

            for (Map sqlScrapWafer:sqlScrapWafers){
                Infos.WaferScrapEventData scrapWafer=new Infos.WaferScrapEventData();
                scrapWafers.add(scrapWafer);

                scrapWafer.setWaferID(convert(sqlScrapWafer.get("WAFER_ID")));
                scrapWafer.setReasonCodeID(objectIdentifier(convert(sqlScrapWafer.get("REASON_ID")),
                        convert(sqlScrapWafer.get("REASON_OBJ"))));
                scrapWafer.setControlWaferFlag(convertB(sqlScrapWafer.get("NPW_WAFER")));
            }

            theEventData.setReasonRouteID(convert(sqlData.get("REASON_PROCESS_ID")));
            theEventData.setReasonOperationNumber(convert(sqlData.get("REASON_OPE_NO")));
            theEventData.setReasonOperationID(convert(sqlData.get("REASON_STEP_ID")));
            theEventData.setReasonOperationPassCount(convertL(sqlData.get("REASON_PASS_COUNT")));
    ...
    public List<Infos.UserDataSet> allUserDataSets(String refKey) {
        String sql="SELECT * FROM OMEVWFSCR_CDA WHERE REFKEY=?";
