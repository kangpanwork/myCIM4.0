...
public class DurableHoldHistoryService {
     ...
     * @date 2019/7/1 10:38
     */
    public Infos.DurableHoldEventRecord getEventData(String id) {
        String sql="Select * from OMEVDURHOLD where id=?";
        List<Map> sqlDatas = baseCore.queryAllForMap(sql, id);
        Infos.DurableHoldEventRecord theEventData=new Infos.DurableHoldEventRecord();
        ...
        List<Infos.DurableHoldEventData> holdRecords=new ArrayList<>();
        ...
        for (Map<String,Object> sqlData:sqlDatas) {
            ...
            sql="SELECT * FROM OMEVDURHOLD_HOLD WHERE REFKEY=?";
            List<Map> sqlHoldRecords = baseCore.queryAllForMap(sql, id);

            for (Map sqlHoldRecord:sqlHoldRecords){
                Infos.DurableHoldEventData holdRecord=new Infos.DurableHoldEventData();
                holdRecords.add(holdRecord);

                holdRecord.setMovementFlag(convertB(sqlHoldRecord.get("MOVE_FLAG")));
                holdRecord.setChangeStateFlag(convertB(sqlHoldRecord.get("STATE_CHG_FLAG")));
                holdRecord.setHoldType(convert(sqlHoldRecord.get("HOLD_TYPE")));
                holdRecord.setHoldReasonCodeID(objectIdentifier(convert(sqlHoldRecord.get("HOLD_REASON_ID")),
                        convert(sqlHoldRecord.get("HOLD_REASON_RKEY"))));
                holdRecord.setHoldUserID(convert(sqlHoldRecord.get("HOLD_USER_ID")));
                holdRecord.setHoldTimeStamp(convert(sqlHoldRecord.get("HOLD_TIME")));
                holdRecord.setResponsibleOperationFlag(convertB(sqlHoldRecord.get("RESP_OPE_FLAG")));
                holdRecord.setResponsibleRouteID(objectIdentifier(convert(sqlHoldRecord.get("RESP_PROCESS_ID")),
                        convert(sqlHoldRecord.get("RESP_PROCESS_RKEY"))));
                holdRecord.setResponsibleOperationID(objectIdentifier(convert(sqlHoldRecord.get("RESP_STEP_ID")),
                        convert(sqlHoldRecord.get("RESP_STEP_RKEY"))));
                holdRecord.setResponsibleOperationNumber(convert(sqlHoldRecord.get("RESP_OPE_NO")));
                holdRecord.setResponsibleOperationName(convert(sqlHoldRecord.get("RESP_OPE_NAME")));
                holdRecord.setHoldClaimMemo(convert(sqlHoldRecord.get("HOLD_TRX_MEMO")));
                holdRecord.setResponsibleOperationExistFlag(convertB(sqlHoldRecord.get("RESP_OPE_EXIST_FLAG")));
            }
    ...
    public List<Infos.UserDataSet> allUserDataSets(String refKey) {
        String sql="SELECT * FROM OMEVDURHOLD_CDA WHERE REFKEY=?";
